...
let showDropLocations = cardt => {
    ...
    if (cardt) {
        ...
        let rule = (pileid, cid) => {
            let cssrule = `[id*="${pileid}"] CARDTS-CARD:last-child[cid="${cid}"] img{--cardBorderColor:yellowgreen;scale:1.1}`;
            return cssrule;
        };
...
let draggingFromPile = false;
let draggingOverPile = false;
let dragMouseMove = false;
let dragMouseUp = false;

let showCardBorderAndSequence = () => {
    let element = SelectionSequenceCards;
    document.addEventListener('mousemove', evt => {
        element.style.setProperty('--SelectionBorder', '2px solid ' + (draggingFromPile ? 'transparent' : 'yellowgreen'));
...
let initDraggingPile = draggedElement => {
    draggingFromPile = draggedElement.closest('CARDTS-ZONE');
    ...
    showDropLocations(DraggingPile.firstChild);
}
function handleDragStart(evt) {
    evt.preventDefault();
    //this , evt.target
    //replease dragging object with a Blank IMG
...
let moveDraggingPile = evt => {
    ...
    if (evt && dragElement.children.length) {
        ...
        if (overPile && overPile.id !== draggingOverPile.id) {
            ...
            let firstCard = cardData(dragElement.firstChild.id);
...
let calculatedCardWidth = getComputedStyle(TablePile1.firstChild).width;
